...
purpose_df.to_csv("/content/AIHUB대화데이터.csv", index=False)
...
get_ipython().system('unzip "/content/drive/MyDrive/AICOSS/02_squad_질문_답변_제시문_말뭉치.zip"')
...
import json
common_sense = open(f"/content/ko_wiki_v1_squad.json", encoding="UTF-8")
...
common_sense_df.to_csv("/content/일반상식.csv", index=False)
...
# JVM 메모리 크기 설정 (2GB)
komoran = Komoran(max_heap_size='2g')
